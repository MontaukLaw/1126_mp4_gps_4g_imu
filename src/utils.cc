...
void find_oldest_file_name_prefix(char *file_name_buf)
{
    ...
    sprintf(file_name_buf, "%s%lld.mp4", VIDEO_FOLDER, oldest_file_name_prefix);
}
...
{
    int total_number = 0;
    list_files(VIDEO_FOLDER, &total_number);
...
void get_file_name_by_time(char *file_name_buf)
{
    ...
    sprintf(file_name_buf, "%s%lld.mp4", VIDEO_FOLDER, time_long);
