...
void init_video_audio_track(uint8_t *pData)
{
    ...
    videoTrack = MP4AddH264VideoTrack(hMP4File, VIDEO_TRACK_TIME_SCALE, VIDEO_TRACK_TIME_SCALE / VIDEO_FPS, 1920, 1080,
...
RK_S32 record_mp4(MEDIA_BUFFER mb)
{
    ...
    if (VENC_NALU_IDRSLICE == frameType)
    {
        ...
        if (pData[0] == 0 && pData[1] == 0 && pData[2] == 0 && pData[3] == 0x01 && pData[4] == 0x67)
        {
            ...
            // 或者是时间到了, 就创建mp4文件
            if (finishFileTimeCounter >= g_seconds_per_file || spsflag == 0x00)
            {
